    ...
    public class MessagesController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Post([FromBody]Activity activity)
        {
            if (activity.Type == ActivityTypes.Message) //if type is message
            {
                ...
                {
                    if (userData.GetProperty<bool>("usernameInserted")) //username has been inserted
                    {

                        if (userData.GetProperty<bool>("passwordInserted"))
                        {
                            Activity infoReply = activity.CreateReply("YAY");
                            await connector.Conversations.ReplyToActivityAsync(infoReply);
                        }
                        else
                        {
                            //save username
                            userData.SetProperty<string>("text", userMessage);
                            ...
                            // List<CardImage> cardImages = new List<CardImage>();
                            //cardImages.Add(new CardImage(url: "http://pngimg.com/upload/small/key_PNG1180.png"));

                            ThumbnailCard plCard = new ThumbnailCard()
                            {
                                Title = "",
                                Subtitle = "Please enter your passphase",
                            };
                            ...
                            userData.SetProperty<bool>("passwordInserted", true);
                            await stateClient.BotState.SetUserDataAsync(activity.ChannelId, activity.From.Id, userData);
                    ...
                    else
                    {
                        ...
                        ThumbnailCard plCard = new ThumbnailCard()
                        {
                            Title = "Contoso Bot Banking",
                            Subtitle = "Hi, please enter your username to begin",
                            // Images = cardImages,
                        };
                        ...
                        userData.SetProperty<bool>("usernameInserted", true);
                        await stateClient.BotState.SetUserDataAsync(activity.ChannelId, activity.From.Id, userData);
                ...
                if (userMessage.ToLower().Equals("hi"))
                {
                ...
                if (!isWeatherRequest)
                {
                    Activity infoReply2 = activity.CreateReply(endOutput);
